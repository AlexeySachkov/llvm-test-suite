...
int main() {
  // Single task invocation methods
  {
    ...
    // OpenCL interoperability kernel invocation
    {
      ...
      err = clEnqueueWriteBuffer(clQ, clBuffer, CL_TRUE, 0, sizeof(int), &data,
                                 0, NULL, NULL);
    ...
    }
    {
      ...
      prog.build_with_source("kernel void sampler_args(int a, sampler_t first, "
  ...
  // Parallel for with range
  {
    ...
    cl_int err;
    {
      ...
      q.submit([&](sycl::handler &cgh) {
        const int value = 1;
        auto local_acc = sycl::accessor<int, 1, sycl::access::mode::read_write,
      ...
      err = clEnqueueReadBuffer(clQ, clBuffer, CL_TRUE, 0,
                                sizeof(int) * dataVec.size(), dataVec.data(), 0,
                                NULL, NULL);
      clReleaseCommandQueue(clQ);
      clReleaseContext(clCtx);
      assert(err == CL_SUCCESS);
